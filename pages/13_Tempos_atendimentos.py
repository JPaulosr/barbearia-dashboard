...
combo_grouped["Categoria"] = combo_grouped["Tipo"].apply(lambda x: "Combo" if any(palavra.lower().strip() in ["combo", "combo+barba", "combo+sombrancelha"] or "," in x for palavra in str(x).split(",")) else "Simples")

df_tempo = combo_grouped.dropna(subset=["Duração (min)"]).copy()

st.markdown(f"<small><i>Registros válidos carregados: {len(df_tempo)}</i></small>", unsafe_allow_html=True)
