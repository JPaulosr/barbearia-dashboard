...
# Flags
# =========================================================
SHOW_TG_TEST = True   # <-- coloque False para esconder o botão de teste
...
def tg_send(text):
    if not _has_telegram():
        st.warning("Telegram não configurado em st.secrets.")
        return
    try:
...
if SHOW_TG_TEST:
    with st.expander("🔔 Teste do Telegram"):
        st.caption("Envia uma mensagem simples para validar TOKEN e CHAT_ID.")
        if st.button("▶️ Enviar ping de teste"):
            tg_send(f"Ping de teste • {now_br()}")
            st.success("Se o Telegram estiver configurado, você deve receber a mensagem agora 😉")
...
if not modo_lote:
    ...
    if st.button("💾 Salvar TODOS atendimentos"):
        if not lista_final:
            ...
            novas = []
            clientes_salvos = set()
            clientes_vinicius = set()  # quem deve receber card
            ...
            if not novas:
                ...
                if enviar_telegram_vinic and clientes_vinicius:
                    st.info(f"📨 Enviando cards para: {', '.join(sorted(clientes_vinicius))}")
                    for cli in sorted(clientes_vinicius):
