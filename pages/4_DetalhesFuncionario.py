...
if len(funcionarios_ano) == 2:
    ...
    df_pivot["Total_Receita"] = df_pivot[[f"Receita_Total_{func}" for func in funcionarios_ano]].sum(axis=1)
    df_pivot["Total_Receita_Formatado"] = df_pivot["Total_Receita"].apply(lambda x: f"R$ {x:,.2f}".replace(",", "v").replace(".", ",").replace("v", "."))
    df_pivot["Diferença Formatada"] = (df_pivot[f"Receita_Total_{funcionarios_ano[0]}"] - df_pivot[f"Receita_Total_{funcionarios_ano[1]}"]).apply(
