...
# Corrigir agrupamento por atendimento (Cliente + Data + Funcionário)
df_atendimentos = df_filtrado.groupby(["Cliente", "Data", "Funcionário"]).agg(Valor=("Valor", "sum")).reset_index()
...
receita_mensal = df_atendimentos.groupby(["Funcionário", "Mês", "Mês_Nome"])["Valor"].sum().reset_index()
...
receita_total = df_atendimentos.groupby("Funcionário")["Valor"].sum().reset_index()
