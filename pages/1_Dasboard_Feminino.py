...
# GID da aba Feminino (pelo print que você mandou)
...
def carregar_dados():
    ...
    # Padronizações
    # Colunas esperadas: Data, Serviço, Valor, Conta, Cliente, Combo, Funcionário, Fase, Tipo, Período
    # Converte Data
...
if df_raw.empty or df_raw["Data"].isna().all():
...
with st.sidebar:
    ...
    def unique_sorted(col):
        return sorted([x for x in df_raw[col].dropna().unique() if str(x).strip() != ""], key=lambda s: s.lower())
...
def contar_atendimentos(df_in):
    if "Cliente" not in df_in or "Data" not in df_in:
        return 0
...
if "Serviço" in df and "Valor" in df:
...
if "Funcionário" in df and "Valor" in df:
...
fig_conta = None
if "Conta" in df and "Valor" in df and not df["Conta"].isna().all():
...
if "Cliente" in df_rank and "Valor" in df_rank:
    top_clientes = (
    ...
    with colB:
        # Exporta a base filtrada completa
        st.download_button(
...
if "Tipo" in df and "Valor" in df:
    mix = df.groupby("Tipo", as_index=False)["Valor"].sum().sort_values("Valor", ascending=False)
    if not mix.empty:
        maior = mix.iloc[0]
        insights.append(f"Maior receita no período veio de **{maior['Tipo']}** (R$ {maior['Valor']:,.2f}).".replace(",", "X").replace(".", ",").replace("X", "."))

# Serviço campeão
if "Serviço" in df and "Valor" in df and not df["Serviço"].isna().all():
...
if "Funcionário" in df and "Valor" in df and not df["Funcionário"].isna().all():
