...
def carregar_dados_google_sheets():
    ...
    # 🧹 Remove as linhas de agrupamento como "Dados do ano 2021"
    ...
    # 🕒 Conversão de horários (preserva segundos)
...
# Função para calcular ociosidade
@st.cache_data
def calcular_ociosidade(df):
    df = df.sort_values(["Funcionário", "Data Group", "Hora Início dt"])
    df["Ociosidade (min)"] = df.groupby(["Funcionário", "Data Group"])["Hora Início dt"].diff().dt.total_seconds().div(60)
    df["Ociosidade (min)"] = df["Ociosidade (min)"].fillna(0)
    return df
