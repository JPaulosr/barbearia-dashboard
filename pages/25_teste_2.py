...
# ---------- 3) Em aberto & exportação ----------
    ...
    if df_base.empty:
        ...
        if em_aberto.empty:
            ...
            st.dataframe(
                resumo.sort_values(["MaxAtraso","ValorTotal"], ascending=[False, False])[["IDLancFiado","Cliente","ValorTotal","QtdeServicos","Combo","Situação"]],
            ...
            # Exportação (Excel via openpyxl, senão CSV)
            try:
