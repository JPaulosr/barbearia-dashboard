...
VALORES_PADRAO = {"Corte":25.0,"Pezinho":7.0,"Barba":15.0,"Sobrancelha":7.0,"Luzes":45.0,"Pintura":35.0,
                  "Alisamento":40.0,"Gel":10.0,"Pomada":15.0}
...
def _get_secret(name: str, default: str | None = None) -> str | None:
    try:
        val = st.secrets.get(name); val = (val or "").strip()
        if val: return val
    except Exception: pass
    return (default or "").strip() or None

def _get_token() -> str | None: return _get_secret("TELEGRAM_TOKEN", TELEGRAM_TOKEN_CONST)
def _get_chat_id_jp() -> str | None: return _get_secret("TELEGRAM_CHAT_ID_JPAULO", TELEGRAM_CHAT_ID_JPAULO_CONST)
def _get_chat_id_vini() -> str | None: return _get_secret("TELEGRAM_CHAT_ID_VINICIUS", TELEGRAM_CHAT_ID_VINICIUS_CONST)
def _check_tg_ready(token: str | None, chat_id: str | None) -> bool: return bool((token or "").strip() and (chat_id or "").strip())
def _chat_id_por_func(funcionario: str) -> str | None: return _get_chat_id_vini() if str(funcionario).strip()=="Vinicius" else _get_chat_id_jp()
...
def tg_send(text: str, chat_id: str | None = None) -> bool:
    ...
    try:
        r=requests.post(f"https://api.telegram.org/bot{token}/sendMessage",
                        json={"chat_id":chat,"text":text,"parse_mode":"HTML","disable_web_page_preview":True}, timeout=30)
        js=r.json(); return bool(r.ok and js.get("ok"))
...
def tg_send_photo(photo_url: str, caption: str, chat_id: str | None = None) -> bool:
    ...
    try:
        r=requests.post(f"https://api.telegram.org/bot{token}/sendPhoto",
                        data={"chat_id":chat,"photo":photo_url,"caption":caption,"parse_mode":"HTML"}, timeout=30)
        js=r.json()
        if r.ok and js.get("ok"): return True
        return tg_send(caption, chat_id=chat)
    except Exception: return tg_send(caption, chat_id=chat)
...
def carregar_fotos_mapa():
    try:
        ...
        df=get_as_dataframe(ws).fillna(""); df.columns=[str(c).strip() for c in df.columns]
        ...
        tmp=df[[cli_col, foto_col]].copy(); tmp.columns=["Cliente","Foto"]; tmp["k"]=tmp["Cliente"].astype(str).map(_norm)
        return {r["k"]:str(r["Foto"]).strip() for _,r in tmp.iterrows() if str(r["Foto"]).strip()}
    except Exception: return {}

def show_foto_cliente(cliente: str):
    try:
        url=FOTOS.get(_norm(cliente or "")); 
        if url: st.image(url, width=160, caption=cliente)
    except Exception: pass

# ============================= Utils =============================
def proxima_terca(d: date) -> date:
    wd=d.weekday(); delta=(1-wd)%7; return d+timedelta(days=delta)

def _fmt_brl(v: float) -> str: return f"R$ {v:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
def _fmt_pct(p: float) -> str: 
    try: return f"{p:.2f}%"
    except Exception: return "-"

def _norm_key(s: str) -> str: return unicodedata.normalize("NFKC", str(s).strip()).casefold()
...
def ensure_headers(ws, desired_headers):
    ...
    if not headers: ws.append_row(desired_headers); return {h:i+1 for i,h in enumerate(desired_headers)}
    missing=[h for h in desired_headers if h not in headers]
    if missing:
        new_headers=headers+missing; ws.update('A1',[new_headers]); headers=new_headers
    return {h:i+1 for i,h in enumerate(headers)}

def append_rows_generic(ws, dicts, default_headers=None):
    headers=ws.row_values(1)
    if not headers: headers=default_headers or sorted({k for d in dicts for k in d.keys()}); ws.append_row(headers)
    hdr_norm=[_norm_key(h) for h in headers]; rows=[]
    for d in dicts:
...
def default_card_flag(conta: str) -> bool:
    ...
    if "nubankcnpj" in s: return False
...
def historico_cliente_por_ano(df_base: pd.DataFrame, cliente: str) -> dict[int, float]:
    ...
    df=df_base.copy(); df["__dt"]=pd.to_datetime(df["Data"], format="%d/%m/%Y", errors="coerce")
...
def breakdown_por_servico_no_ano(df_base: pd.DataFrame, cliente: str, ano: int, max_itens: int = 8):
    ...
    if df.empty: return pd.DataFrame(columns=["Serviço","Qtd","Total"]), 0, 0.0, 0, 0.0
    agg=(df.groupby("Serviço", dropna=True).agg(Qtd=("Serviço","count"), Total=("__valor","sum"))
           .reset_index()).sort_values("Total", ascending=False)
    total_qtd=int(agg["Qtd"].sum()); total_val=float(agg["Total"].sum())
    top=agg.head(max_itens).copy(); outros=agg.iloc[max_itens:] if len(agg)>max_itens else pd.DataFrame(columns=agg.columns)
...
def format_extras_numeric(ws):
    ...
    fmt("ValorBrutoRecebido","NUMBER","0.00"); fmt("ValorLiquidoRecebido","NUMBER","0.00")
    fmt("TaxaCartaoValor","NUMBER","0.00"); fmt("TaxaCartaoPct","PERCENT","0.00%")

# ---------- NOVO: datas de registro a partir do ID ----------
def _so_digitos(s: str) -> str: return "".join(ch for ch in str(s) if ch.isdigit())
...
def periodo_do_id(df: pd.DataFrame, idl: str) -> str:
    try:
        vals=(df.loc[df["IDLancFiado"]==idl,"Período"].dropna().astype(str).str.strip())
        vals=[v for v in vals if v]; uniq=sorted(set(vals))
        return uniq[0] if len(uniq)==1 else ""
    except Exception: return ""

# ---------- Resumo visitas (para o card do pagamento) ----------
def _resumo_visitas(df_base: pd.DataFrame, cliente: str):
    ...
    df=df_base.copy(); df["__dt"]=pd.to_datetime(df.get("Data"), format=DATA_FMT, errors="coerce")
    ...
    datas=df_day["__dt"].sort_values().tolist(); total=len(datas)
    ...
    dist=(date.today()-datas[-1].date()).days if total else None
    return media, dist, total, (ultimo_func or "-")

# ============================= Sheets =============================
def garantir_aba(ss, nome, cols):
    try: ws=ss.worksheet(nome)
    except gspread.WorksheetNotFound:
        ws=ss.add_worksheet(title=nome, rows=200, cols=max(10, len(cols))); ws.append_row(cols); return ws
    existing=ws.row_values(1); 
    if not existing: ws.append_row(cols)
    return ws
...
def carregar_listas():
    ss=conectar_sheets()
    ws_base=garantir_aba(ss, ABA_BASE, BASE_COLS_ALL); ensure_headers(ws_base, BASE_COLS_ALL)
...
def gerar_id(prefixo): return f"{prefixo}-{datetime.now(TZ).strftime('%Y%m%d%H%M%S%f')[:-3]}"
...
if acao == "➕ Lançar fiado":
    ...
    with tab_uni:
        ...
        with c1:
            ...
            if not cliente: cliente = st.text_input("Ou digite o nome do cliente","", key="fiado_cli_txt_uni")
            if cliente: show_foto_cliente(cliente)
        ...
        with c2:
            data_atend = st.date_input("Data do atendimento", value=date.today(), key="fiado_data_uni")
            venc = st.date_input("Vencimento (opcional)", value=date.today(), key="fiado_venc_uni")
        ...
        if servicos:
            ...
            for s in servicos:
                ...
                valores_custom[s]=st.number_input(f"{s} (padrão: R$ {padrao:.2f})",
                                                  value=float(padrao), step=1.0, format="%.2f", key=f"valor_{s}_uni")

        if st.button("Salvar fiado (único)", use_container_width=True, key="btn_salvar_uni"):
            if not cliente: st.error("Informe o cliente.")
            elif not servicos: st.error("Informe combo ou um serviço.")
            else:
                idl=gerar_id("L"); data_str=data_atend.strftime(DATA_FMT); venc_str=venc.strftime(DATA_FMT) if venc else ""
                novas=[]
                for s in servicos:
                    valor_item=float(valores_custom.get(s, VALORES_PADRAO.get(s,0.0)))
                    novas.append({"Data":data_str,"Serviço":s,"Valor":valor_item,"Conta":"Fiado",
                                  "Cliente":cliente,"Combo":(combo_str if combo_str else ""),"Funcionário":funcionario,
                                  "Fase":fase,"Tipo":tipo,"Período":periodo,"StatusFiado":"Em aberto",
                                  "IDLancFiado":idl,"VencimentoFiado":venc_str,"DataPagamento":"",
                ...
                append_rows_generic(ws_l, [{"IDLanc":idl,"Data":data_str,"Cliente":cliente,"Combo":combo_str,
                ...
                try:
                    ...
                    msg_html=("🧾 <b>Novo fiado criado</b>\n"
                              ...
                              f"🆔 ID: <code>{idl}</code>")
                    chat_dest=_chat_id_por_func(funcionario); foto=FOTOS.get(_norm(cliente))
    ...
    with tab_lote:
        st.caption("💡 Preencha várias linhas e clique em **Salvar fiados (lote)**. "
                   "Se for serviço único, o campo Valor aparece; para combo usa a tabela padrão.")
        ...
        df_modelo=pd.DataFrame({
            ...
            "Data":[date.today() for _ in range(num_linhas)],
            "Vencimento":[date.today() for _ in range(num_linhas)],
        ...
        with col_editor:
            edited=st.data_editor(df_modelo, num_rows="dynamic", use_container_width=True, key="fiado_editor_lote",
                column_config={
                    "Cliente": st.column_config.SelectboxColumn(options=[""]+clientes, help="Escolha um cliente já cadastrado"),
                    ...
                    "Combo_ou_Serviço": st.column_config.SelectboxColumn(options=[""]+opcoes_combo_serv,
                        help="Escolha um combo existente (ex.: corte+barba) OU um serviço único"),
                    "Valor": st.column_config.NumberColumn(help="Para serviço único, edite o valor. Para combos é ignorado.",
        ...
        with col_foto:
            try:
                ...
                if primeira_linha_cli:
                    st.caption("Foto do cliente"); show_foto_cliente(primeira_linha_cli)
            except Exception: pass

        try:
            for idx in edited.index:
                nome=str(edited.at[idx,"Combo_ou_Serviço"]).strip()
                if not nome:
                    edited.at[idx,"Combo_ou_Serviço"]="Corte"; nome="Corte"
                if "+" not in nome and nome in VALORES_PADRAO:
                    if not float(edited.at[idx,"Valor"] or 0.0):
        ...
        if st.button("Salvar fiados (lote)", use_container_width=True, key="btn_salvar_lote"):
            ...
            if linhas_validas.empty: st.error("Preencha pelo menos uma linha com Cliente e Combo_ou_Serviço."); st.stop()
            clientes_ok={str(c).strip() for c in clientes}
            invalidos=sorted({str(c).strip() for c in linhas_validas["Cliente"].tolist() if str(c).strip() not in clientes_ok})
            if invalidos: st.error("Há cliente(s) não cadastrados no lote: "+", ".join(invalidos)+". Corrija no seletor."); st.stop()
            ...
            for _, r in linhas_validas.iterrows():
                ...
                data_i=r["Data"] if isinstance(r["Data"], date) else date.today()
                venc_i=r["Vencimento"] if isinstance(r["Vencimento"], date) else date.today()
                ...
                if "+" in escolha_i:
                    servicos_i=parse_combo(escolha_i) or []; combo_str=escolha_i
                else:
                    servicos_i=[escolha_i]; combo_str=""

                if not servicos_i: continue

                idl=gerar_id("L"); data_str=data_i.strftime(DATA_FMT); venc_str=venc_i.strftime(DATA_FMT) if venc_i else ""
                novas=[]
                for s in servicos_i:
                    ...
                    novas.append({"Data":data_str,"Serviço":s,"Valor":valor_item,"Conta":"Fiado",
                                  ...
                                  "StatusFiado":"Em aberto","IDLancFiado":idl,"VencimentoFiado":venc_str,"DataPagamento":"",
                                  "ValorBrutoRecebido":"","ValorLiquidoRecebido":"","TaxaCartaoValor":"","TaxaCartaoPct":"",
                                  "FormaPagDetalhe":"","PagamentoID":""})
                append_rows_base(ws_base, novas)
                total=float(pd.to_numeric(pd.DataFrame(novas)["Valor"], errors="coerce").fillna(0).sum())
                append_rows_generic(ws_l,[{"IDLanc":idl,"Data":data_str,"Cliente":cliente_i,"Combo":(combo_str if combo_str else ""),
                ...
                try:
                    ...
                    msg_html=("🧾 <b>Novo fiado criado</b>\n"
                              ...
                              f"⏳ Vencimento: {venc_str or '-'}\n"
                              f"🆔 ID: <code>{idl}</code>")
                    chat_dest=_chat_id_por_func(funcionario_i); foto=FOTOS.get(_norm(cliente_i))
                    if foto: tg_send_photo(foto, msg_html, chat_id=chat_dest)
                    else: tg_send(msg_html, chat_id=chat_dest)
                except Exception: pass
...
# ---------- 2) Registrar pagamento ----------
    ...
    with colc1:
        ...
        if cliente_sel: show_foto_cliente(cliente_sel)
    ultima=ultima_forma_pagto_cliente(df_base_full, cliente_sel) if cliente_sel else None
    lista_contas_default=["Pix","Dinheiro","Cartão","Transferência","Pagseguro","Mercado Pago","Nubank CNPJ","SumUp","Cielo","Stone","Getnet","Outro","Nubank"]
    ...
    if cliente_sel:
        ...
        if modo_sel.startswith("Por ID"):
            ...
            def atraso_max(idval):
                ...
                try: vdt=pd.to_datetime(v.iloc[0], format=DATA_FMT, errors="coerce").date() if not v.empty else None
                except Exception: vdt=None
                if vdt:
            ...
            for _, r in resumo_ids.iterrows():
                atraso=atraso_max(r["IDLancFiado"]); badge="Em dia" if atraso<=0 else f"{int(atraso)}d atraso"
                dt_reg=data_reg_do_id(r["IDLancFiado"]); periodo_id=periodo_do_id(grupo_cli, r["IDLancFiado"])
                partes=[r["IDLancFiado"]]
                if dt_reg: partes.append(f"reg: {dt_reg.strftime(DATA_FMT)}")
                if periodo_id: partes.append(periodo_id)
            ...
            ids_valores=[i[0] for i in ids_opcoes]; labels_id={i:l for i,l in ids_opcoes}
            select_all_ids=st.checkbox("Selecionar todos os fiados deste cliente", value=False, disabled=not bool(ids_valores))
            id_selecionados=st.multiselect("Selecione 1 ou mais fiados do cliente", options=ids_valores,
                                           default=(ids_valores if select_all_ids else []),
                                           format_func=lambda x: labels_id.get(x,x))
        else:
            linhas_cli=grupo_cli.copy(); linhas_cli["IdxBase"]=linhas_cli.index
            ...
            linhas_indices_sel=st.multiselect("Selecione linhas específicas do cliente (por serviço)",
                                              options=linhas_todas, default=(linhas_todas if select_all_linhas else []),
                                              format_func=lambda i: linhas_label_map.get(i,str(i)))

    cold1, cold2 = st.columns(2)
    with cold1: data_pag = st.date_input("Data do pagamento", value=date.today())
    with cold2: obs = st.text_input("Observação (opcional)","", key="obs")

    total_sel = 0.0
    valor_liquido_cartao = None; bandeira_cartao = ""; tipo_cartao="Crédito"; parcelas_cartao=1
    taxa_valor_est = 0.0; taxa_pct_est = 0.0; subset_preview = pd.DataFrame()
    ...
    if not subset_preview.empty:
        ...
        if usar_cartao:
            with st.expander("💳 Detalhes da maquininha (informe o LÍQUIDO)", expanded=True):
                ...
                with cdc1:
                    valor_liquido_cartao = st.number_input("Valor recebido (líquido da maquininha)",
                                                           value=float(total_sel), step=1.0, format="%.2f")
        ...
        st.caption("Resumo por serviço selecionado:"); st.dataframe(resumo_srv, use_container_width=True, hide_index=True)
    ...
    if st.button("Registrar pagamento", use_container_width=True, disabled=disabled_btn):
        dfb, ws_base2 = read_base_raw(ss); ensure_headers(ws_base2, BASE_COLS_ALL); format_extras_numeric(ws_base2)
        mask = dfb.get("IDLancFiado","").isin(id_selecionados) if modo_sel.startswith("Por ID") else dfb.index.isin(linhas_indices_sel)
        if not mask.any(): st.error("Nenhuma linha encontrada para a seleção feita.")
        else:
            subset_all=dfb[mask].copy(); subset_all["Valor"]=pd.to_numeric(subset_all["Valor"], errors="coerce").fillna(0)
            total_bruto=float(subset_all["Valor"].sum()); data_pag_str=data_pag.strftime(DATA_FMT)
            id_pag=f"P-{datetime.now(TZ).strftime('%Y%m%d%H%M%S%f')[:-3]}"
            total_liquido=float(valor_liquido_cartao or total_bruto) if usar_cartao else total_bruto
            ...
            headers_map=col_map(ws_base2); updates=[]; liq_acum=0.0; idxs=list(subset_all.index)
            for i, idx in enumerate(idxs):
                row_no=int(idx)+2; bruto_i=float(subset_all.loc[idx,"Valor"])
                ...
                liq_acum+=liq_i; taxa_i=round(bruto_i - liq_i, 2); taxa_pct_i=(taxa_i/bruto_i*100.0) if bruto_i>0 else 0.0
                pairs={"Conta":forma_pag,"StatusFiado":"Pago","VencimentoFiado":"","DataPagamento":data_pag_str,"Valor":liq_i,
                       "ValorBrutoRecebido":(bruto_i if usar_cartao else ""),"ValorLiquidoRecebido":(liq_i if usar_cartao else ""),
                       "TaxaCartaoValor":(taxa_i if usar_cartao else ""),"TaxaCartaoPct":(round(taxa_pct_i,4) if usar_cartao else ""),
            ...
            if usar_cartao:
                try:
                    ws_taxas=garantir_aba(ss, ABA_TAXAS, TAXAS_COLS); ensure_headers(ws_taxas, TAXAS_COLS)
                    append_rows_generic(ws_taxas,[{"IDPagamento":id_pag,"Cliente":cliente_sel,"DataPag":data_pag_str,
                                                   "Bandeira":bandeira_cartao,"Tipo":tipo_cartao,"Parcelas":int(parcelas_cartao),
                                                   "Bruto":total_bruto,"Liquido":total_liquido,
            ...
            ws_p=garantir_aba(ss, ABA_PAGT, PAGT_COLS); ensure_headers(ws_p, PAGT_COLS)
            append_rows_generic(ws_p,[{"IDPagamento":id_pag,"IDLancs":";".join(sorted(set(subset_all["IDLancFiado"].astype(str)))),
            ...
            st.success(f"Pagamento registrado para **{cliente_sel}**. Total líquido: {_fmt_brl(total_liquido)} (bruto {_fmt_brl(total_bruto)}).")
            st.cache_data.clear()

            # ---- Mensagens Telegram (cópia para controle enriquecida) ----
            try:
                ...
                if len(set(datas_sel))==1: data_atend_txt=next(iter(set(datas_sel))).strftime(DATA_FMT)
                ...
                bloco_atend=("📌 <b>Atendimento registrado</b>\n"
                ...
                if not brk.empty:
                    linhas_srv="\n".join(f"• {r['Serviço']}: {int(r['Qtd'])}× · <b>{_fmt_brl(float(r['Total']))}</b>" for _,r in brk.iterrows())
                    if oq>0: linhas_srv+=f"\n• Outros: {oq}× · <b>{_fmt_brl(ov)}</b>"
                    bloco_srv=f"\n\n🔎 <b>{ano_corr}: por serviço</b>\n{linhas_srv}\nTotal ({ano_corr}): <b>{_fmt_brl(tv)}</b>"
                else: bloco_srv=f"\n\n🔎 <b>{ano_corr}: por serviço</b>\n• (sem registros)"
                msg_jp=("🧾 <b>Cópia para controle</b>\n"+bloco_atend+
                        f"\n💳 Forma: <b>{forma_pag}</b>\n"
                        f"💵 Bruto: <b>{_fmt_brl(total_bruto)}</b> · Líquido: <b>{_fmt_brl(total_liquido)}</b>"+
                        linha_taxa_cp+bloco_hist_indic+bloco_hist+bloco_srv+(f"\n\n📝 Obs.: {obs}" if obs else ""))
            ...
            # Comissões
            try:
                sub=subset_all.copy(); sub["Valor"]=pd.to_numeric(sub["Valor"], errors="coerce").fillna(0.0)
                grup=sub.groupby("Funcionário", dropna=True)["Valor"].sum().reset_index()
                itens=[]
                for _, r in grup.iterrows():
                    func_raw=str(r["Funcionário"]).strip()
                    if unicodedata.normalize("NFKC", func_raw).casefold() not in COMISSAO_FUNCIONARIOS: continue
                    comiss=round(float(r["Valor"])*COMISSAO_PERC_PADRAO,2); itens.append(f"• {func_raw}: <b>{_fmt_brl(comiss)}</b>")
                if itens:
                    dt_pgto=proxima_terca(data_pag)
                    tg_send("💸 <b>Comissões sugeridas</b> "
                            f"({int(COMISSAO_PERC_PADRAO*100)}%)\n"+"\n".join(itens)+
                            f"\n📌 Pagar na próxima terça: <b>{dt_pgto.strftime(DATA_FMT)}</b>", chat_id=_get_chat_id_jp())
            except Exception: pass

# ---------- 3) Em aberto & exportação ----------
elif acao == "📋 Em aberto & exportação":
    st.subheader("📋 Fiados em aberto (agrupados por ID)")
    ss=conectar_sheets(); df_base_full,_=read_base_raw(ss)
    if df_base_full.empty: st.info("Sem dados.")
    else:
        em_aberto=df_base_full[df_base_full.get("StatusFiado","")=="Em aberto"].copy()
        if em_aberto.empty: st.success("Nenhum fiado em aberto 🎉")
        else:
            colf1,colf2 = st.columns([2,1])
            with colf1:
                filtro_cliente=st.text_input("Filtrar por cliente (opcional)","")
                if filtro_cliente.strip():
                    em_aberto=em_aberto[em_aberto["Cliente"].astype(str).str.contains(filtro_cliente.strip(), case=False, na=False)]
            with colf2:
                funcionarios_abertos=sorted(em_aberto["Funcionário"].dropna().astype(str).unique().tolist())
                filtro_func=st.selectbox("Filtrar por funcionário (opcional)", [""]+funcionarios_abertos)
                if filtro_func: em_aberto=em_aberto[em_aberto["Funcionário"]==filtro_func]
            ...
            em_aberto["DiasAtraso"]=em_aberto["__venc"].apply(lambda d: (hoje-d).days if (d is not None and hoje>d) else 0)
            ...
            resumo=(em_aberto.groupby(["IDLancFiado","Cliente"], as_index=False)
            ...
            # NOVO: data e período de registro
            resumo["RegistradoEm"]=resumo["IDLancFiado"].apply(lambda x: (data_reg_do_id(x).strftime(DATA_FMT) if data_reg_do_id(x) else "-"))
            ...
            total=float(resumo["ValorTotal"].sum()); st.metric("Total em aberto", _fmt_brl(total))
...
# ---------- 4) Pagos (histórico) ----------
    ...
    ss=conectar_sheets()
    df_base,_=read_base_raw(ss); df_pagos_base=df_base[df_base.get("StatusFiado","")=="Pago"].copy()
    ...
    def _to_date(s):
        try: return datetime.strptime(str(s), DATA_FMT).date()
        except Exception: return None
    if "DataPagamento" in df_pag.columns: df_pag["__data"]=df_pag["DataPagamento"].apply(_to_date)
    else: df_pag["__data"]=None

    c1,c2,c3 = st.columns([1,1,2])
    with c1: data_ini = st.date_input("De", value=date.today()-timedelta(days=30), key="pagos_ini")
    with c2: data_fim = st.date_input("Até", value=date.today(), key="pagos_fim")
    with c3: filtro_cli = st.text_input("Cliente (contém)","", key="pagos_cli")

    mask=pd.Series([True]*len(df_pag))
    if data_ini: mask &= (df_pag["__data"]>=data_ini)
    if data_fim: mask &= (df_pag["__data"]<=data_fim)
    if filtro_cli.strip(): mask &= df_pag.get("Cliente","").astype(str).str.contains(filtro_cli.strip(), case=False, na=False)

    vis=df_pag[mask].copy()

    # NOVO: datas "RegistradoDe/RegistradoAte" a partir de IDLancs (sem hora)
    def _registrado_de_ate(idl_str: str):
        ids=[s.strip() for s in str(idl_str or "").split(";") if s.strip()]
        datas=[data_reg_do_id(s) for s in ids]; datas=[d for d in datas if d]
        if not datas: return "-", "-"
        return min(datas).strftime(DATA_FMT), max(datas).strftime(DATA_FMT)
    vis["RegistradoDe"], vis["RegistradoAte"] = zip(*vis.get("IDLancs","").apply(_registrado_de_ate))

    try:
        totals={"Total líquido": vis.get("TotalLiquido",0).apply(pd.to_numeric, errors="coerce").fillna(0).sum(),
    ...
    cols_show=["DataPagamento","RegistradoDe","RegistradoAte","Cliente","Forma","TotalBruto","TotalLiquido",
               "TaxaValor","TaxaPct","IDLancs","Obs","IDPagamento"]
    cols_show=[c for c in cols_show if c in vis.columns]
    st.dataframe(vis[cols_show].sort_values("DataPagamento", ascending=False), use_container_width=True, hide_index=True)

    try:
        from openpyxl import Workbook  # noqa
        buf=BytesIO()
        with pd.ExcelWriter(buf, engine="openpyxl") as w:
            vis[cols_show].sort_values("DataPagamento", ascending=False).to_excel(w, index=False, sheet_name="Fiados_Pagos")
    ...
    if cli_pick:
        ...
        with colf1: show_foto_cliente(cli_pick)
        with colf2:
            linhas_cli=df_pagos_base[(df_pagos_base.get("Cliente","")==cli_pick) &
                                     df_pagos_base.get("DataPagamento","").astype(str).str.len().gt(0)].copy()
            if not linhas_cli.empty:
                linhas_cli["Valor"]=pd.to_numeric(linhas_cli["Valor"], errors="coerce").fillna(0.0)
                st.dataframe(linhas_cli[["Data","Serviço","Valor","Conta","Funcionário","IDLancFiado","DataPagamento"]],
                             use_container_width=True, hide_index=True)
