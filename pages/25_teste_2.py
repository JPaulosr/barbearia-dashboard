    ...
    # forma de pagamento (usa as mesmas contas da Base) + sugestão da última usada pelo cliente
    ...
    if cliente_sel:
        ...
        # pega vencimento mais antigo para compor atraso máximo
    ...
    if id_selecionados:
        ...
        # 📊 Resumo por serviço (ex.: 2x Corte, 1x Barba)
        resumo_srv = (subset.groupby("Serviço", as_index=False)
                            .agg(Qtd=("Serviço","count"), Total=("Valor","sum"))
                            .sort_values(["Qtd","Total"], ascending=[False, False]))
        resumo_srv["Total"] = resumo_srv["Total"].map(lambda v: f"R$ {v:,.2f}".replace(",", "X").replace(".", ",").replace("X","."))
