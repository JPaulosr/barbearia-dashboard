...
if len(datas) < 2:
    ...
    if "Duração (min)" in df_cliente.columns and df_cliente["Duração (min)"].notna().any():
    tempo_total = df_cliente["Duração (min)"].sum()
    tempo_total_str = f"{int(tempo_total)} minutos"
else:
    tempo_total_str = "Indisponível"
